...
class CoDExRel10(CoDEx):
    ...
    name = "codex-10"
    def __init__(self, root):
        super(CoDExRel10, self).__init__(root=root, size='10')
        
class CoDExRel5(CoDEx):
    """
    #node: x
    #edge: x
    #relation: 5
    ...
    name = "codex-5"
    def __init__(self, root):
        super(CoDExRel5, self).__init__(root=root, size='10')
...
class InductiveDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        valid_res = self.load_file(
            ...
            inference_res["inv_rel_vocab"] if self.valid_on_inf else train_res["inv_entity_vocab"]
        )
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=num_train_nodes,
                          inv_ent_vocab=list(train_res["inv_entity_vocab"].items()), 
                          inv_rel_vocab=list(train_res["inv_rel_vocab"].items()),
        ...
        valid_data = Data(edge_index=inf_edges if self.valid_on_inf else train_fact_index, 
                          ...
                          inv_ent_vocab=list(valid_res["inv_entity_vocab"].items()), 
                          inv_rel_vocab=list(valid_res["inv_rel_vocab"].items()),
        ...
        test_data = Data(edge_index=inf_edges, edge_type=inf_etypes, num_nodes=inference_num_nodes,
                         inv_ent_vocab=list(test_res["inv_entity_vocab"].items()), 
                          inv_rel_vocab=list(test_res["inv_rel_vocab"].items()),
